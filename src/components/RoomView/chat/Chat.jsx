...
const Chat = () => {
  ...
  */
  ...
  useEffect(() => {
    onValue(chatRef, (snapshot) => {
      if (snapshot.exists()) {
        ...
        //bottomOfMessagesRef.current.scroll(0, 0, messages.offsetBottom);
      } else {
  ...
  useEffect(() => {
    console.log('hi!', bottomOfMessagesRef.current.scrollHeight);
  ...
  const handleKeyDown = (e) => {
    console.log('chat thinks you entered the Enter key!');
    if (e.key === 'Enter') {
      ...
      <div className="chat">
        <div className="messages" ref={bottomOfMessagesRef}>
          {Object.keys(messages).map((message, i) => {
            if (messages[message]['sender'] === username)
              return (
